    ...
    <div className="h-full flex flex-col bg-white border-r">
      ...
      <div className="flex-1 p-4 overflow-y-auto border-b">
        <div className="flex justify-between items-center mb-3">
          <h3 className="font-bold text-gray-700 uppercase text-xs tracking-wider">Roadmaps trong Room</h3>
          <button onClick={handleCreateRoadmap} className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded font-bold hover:bg-blue-200 shadow-sm">+ Mới</button>
        </div>
          ...
          {roadmapFiles.map(file => {
            ...
            return (
              <li 
                ...
                className={`group flex items-center justify-between px-2 py-1.5 rounded cursor-pointer transition-colors ${isActive ? 'bg-blue-50 text-blue-700 font-semibold' : 'hover:bg-gray-100'}`}
              >
                {isEditing ? (
                  <input 
                    autoFocus
                    ...
                    className="flex-1 px-1 py-0.5 border border-blue-400 rounded text-xs outline-none"
                  />
                ) : (
                ...
                {!isEditing && (
                  <div className="hidden group-hover:flex gap-1 shrink-0">
                    <button 
                      ...
                      className="text-gray-400 hover:text-blue-600 px-1" title="Đổi tên"
                    >✏️</button>
                    <button 
                      onClick={(e) => handleDeleteRoadmap(file, e)}
                      className="text-gray-400 hover:text-red-600 px-1" title="Xóa"
                    >🗑️</button>
                  </div>
                )}
              </li>
      ...
      {/* QUẢN LÝ TAGS (Giữ nguyên) */}
      <div className="flex-1 p-4 overflow-y-auto bg-gray-50">
        <h3 className="font-bold text-gray-700 mb-3 uppercase text-xs tracking-wider">Quản lý Tags</h3>
        <form onSubmit={handleAddTag} className="flex gap-2 mb-3">
          <input type="text" value={newTagName} onChange={(e) => setNewTagName(e.target.value)} placeholder="Tên tag mới..." className="flex-1 border rounded px-2 py-1 text-sm focus:outline-blue-500" />
          <button type="submit" className="bg-gray-800 text-white px-2 py-1 rounded text-sm hover:bg-gray-700">+</button>
        </form>
        <div className="flex flex-wrap gap-2">
          {tags.map(tag => (
            <div key={tag.id} className="flex items-center gap-1 bg-white border rounded-full px-2 py-1 shadow-sm text-xs group">
              <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: tag.color }}></span>
              <span className="font-medium text-gray-700">{tag.name}</span>
              <button onClick={() => deleteTag(tag.id)} className="opacity-0 group-hover:opacity-100 text-red-500 hover:text-red-700 ml-1 transition-opacity">×</button>
            </div>
          ))}
        </div>
      </div>
