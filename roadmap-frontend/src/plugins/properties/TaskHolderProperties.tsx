...
export default function TaskHolderProperties({ node }: { node: Node }) {
  ...
  return (
    <div className="flex flex-col gap-4 text-sm">
      <h2 className="font-bold text-lg border-b pb-2 text-indigo-700">Task-Holder Properties</h2>
      
      <div className="flex flex-col gap-1">
        <label className="font-semibold text-gray-600">Tên Nhóm Task:</label>
        <input 
          type="text" 
          value={data.title || ''} 
          onChange={(e) => handleChange('title', e.target.value)}
          className="border rounded p-1.5 focus:outline-indigo-500"
          placeholder="Ví dụ: Phase 1: Design..."
        />
      </div>

      <div className="flex flex-col gap-1">
        <label className="font-semibold text-gray-600">Mô tả (Người dùng định nghĩa):</label>
        <textarea 
          value={data.description || ''} 
          onChange={(e) => handleChange('description', e.target.value)}
          className="border rounded p-1.5 focus:outline-indigo-500 h-20 resize-none"
          placeholder="Ghi chú thêm về nhóm task này..."
        />
      </div>

      <div className="flex flex-col gap-1">
        <label className="font-semibold text-gray-600">Màu nền:</label>
        <input 
          type="color" 
      ...
      {/* Bảng Thống kê Read-only */}
      <div className="mt-2 p-3 bg-indigo-50 rounded-md border border-indigo-100">
        <h3 className="font-bold text-indigo-800 mb-2">Thống kê tiến độ</h3>
